...
inline void freezeIDIF(int cycles){
...
inline bool is_threeOPerand(RISCV::Inst inst){
  if(inst == RISCV::FMADD || inst == RISCV::FMSUB || inst == RISCV::FNM) return true;
  if(inst == RISCV::FMADDU || inst == RISCV::FMSUBU || inst == RISCV::FNMU) return true;
  else return false;
}
#endif
