...
namespace RISCV
{
  ...
  inline int operand_num(int opcode)
  {
    switch (opcode)
    {
    case OP_LUI:
...
private:
  ...
  // Freeze ID IF when having data hazards
  // At this time, data read by DRegNew is wrong, so we need to execute ID with the old FReg again
  // In this way, the FReg new shouldn' t be written, and DRegNew can not be passed to DReg(bubble in execute)
  // 并且FRegNew在一个周期末会被覆写为零，但是PC已经++, 这意味着继续执行的指令会跳过一个，PC的偏移需要修正
  // cycles数是基于ID被暂停的周期数的(IF阶段要多暂停一个周期，不然进行ID的时候原来被暂停的指令就已经不见了)
  void freezeIDIF(int cycles);

  int if_stall_cycles_for_newly_decoded_inst();
};
inline bool is_threeOPerand(RISCV::Inst inst)
{
  if (inst == RISCV::FMADD || inst == RISCV::FMSUB || inst == RISCV::FNM)
    return true;
  if (inst == RISCV::FMADDU || inst == RISCV::FMSUBU || inst == RISCV::FNMU)
    return true;
  else
    return false;
}
